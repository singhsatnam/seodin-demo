...
export class InterviewService {
    ...
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/interviews';

    constructor(private http: HttpClient) { }

    create(interview: Interview): Observable<EntityResponseType> {
        const copy = this.convert(interview);
        return this.http.post<Interview>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(interview: Interview): Observable<EntityResponseType> {
        const copy = this.convert(interview);
        return this.http.put<Interview>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<Interview>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<Interview[]>> {
        const options = createRequestOption(req);
        return this.http.get<Interview[]>(this.resourceUrl, { params: options, observe: 'response' })
    ...
    search(req?: any): Observable<HttpResponse<Interview[]>> {
        const options = createRequestOption(req);
        return this.http.get<Interview[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Interview[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertArrayResponse(res: HttpResponse<Interview[]>): HttpResponse<Interview[]> {
