...
export class VideoService {
    ...
    search(req?: any): Observable<HttpResponse<Video[]>> {
        const options = createRequestOption(req);
        return this.http.get<Video[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Video[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(video: Video): Video {
        ...
        copy.recorded = this.dateUtils
            .convertLocalDateFromServer(video.recorded);
        return copy;
    }
    ...
    private convert(video: Video): Video {
        const copy: Video = Object.assign({}, video);
        copy.recorded = this.dateUtils
            .convertLocalDateToServer(video.recorded);
        return copy;
    }
}
